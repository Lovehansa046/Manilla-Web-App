"use client"

import Navbar from "@/app/components/Navbar";
import Footer from "@/app/components/Footer";
import ProductGrid from "@/app/components/ProductPage"


const sampleProducts = [
    {
        name: 'Капучино',
        description: 'Ароматный кофе с молочной пенкой.',
        price: 3.99,
        image: 'https://via.placeholder.com/400'
    },
    {
        name: 'Латте',
        description: 'Кофе с мягким молочным вкусом.',
        price: 4.49,
        image: 'https://via.placeholder.com/400'
    },
    {
        name: 'Американо',
        description: 'Классический чёрный кофе.',
        price: 2.99,
        image: 'https://via.placeholder.com/400'
    },
    {
        name: 'Эспрессо',
        description: 'Сильный и насыщенный кофе.',
        price: 2.49,
        image: 'https://via.placeholder.com/400'
    },
    {
        name: 'Мохито',
        description: 'Освежающий напиток с мятой и лаймом.',
        price: 5.99,
        image: 'https://via.placeholder.com/400'
    },
    {
        name: 'Смузи Манго',
        description: 'Фруктовый напиток с манго.',
        price: 4.99,
        image: 'https://via.placeholder.com/400'
    },
    {
        name: 'Салат Цезарь',
        description: 'Классический салат с курицей.',
        price: 7.99,
        image: 'https://via.placeholder.com/400'
    },
    {name: 'Бургер', description: 'Сочный бургер с говядиной.', price: 8.49, image: 'https://via.placeholder.com/400'},
    {
        name: 'Пицца Пепперони',
        description: 'Острая пицца с пепперони.',
        price: 9.99,
        image: 'https://via.placeholder.com/400'
    },
    {
        name: 'Круассан',
        description: 'Свежая французская выпечка.',
        price: 2.49,
        image: 'https://via.placeholder.com/400'
    },
];

export default function NonAlcoholPage() {
    return (
        <>
            <Navbar></Navbar>

            <ProductGrid products={sampleProducts}/>

            <Footer></Footer>
        </>
    )
}